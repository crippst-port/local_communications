 ...
 * profile page - see local_feedback_myprofile_navigation() in lib.php.
 *
...
$PAGE->set_url('/local/feedback/my_submissions.php');
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('mysubmissions_heading', 'local_feedback'));
$PAGE->set_heading(fullname($USER));
$PAGE->requires->css('/local/feedback/styles.css');

$table = new submissions_table(
    'local-feedback-my-submissions',
    new moodle_url('/local/feedback/my_submissions.php'),
    0, '', false, '', 0, [], $USER->id, true
);
$table->is_downloading($download, 'my_feedback');
$table->show_download_buttons_at([TABLE_P_TOP]);

$total = $DB->count_records('local_feedback_submissions', ['userid' => $USER->id]);

if (!$table->is_downloading()) {
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('mysubmissions_heading', 'local_feedback'));

    if ($total === 0) {
        echo $OUTPUT->notification(get_string('mysubmissions_none', 'local_feedback'), 'info');
    }
}
